...
- **3点連携**:
  ...
  2. **HTMLファイル** → VS Codeで開く（🟡黄色ハイライト）
  3. **ブラウザ** → 該当要素を🔴赤枠ハイライト
- **モバイルCSS検索**:
  - ビューポート幅 **767px以下** → 自動的に `@media (max-width: 767px)` 内を優先検索
  - **Ctrl+ダブルクリック** → 強制モバイルCSS検索（PC表示時も可）
- **前提条件**:
  - Chrome拡張（css-jumper）インストール済み
  - Native Messaging設定済み（setup.bat実行済み）
  - ポート3848でHTTPサーバー起動（自動）
- **⚠ 制限**: DevToolsレスポンシブモード中のダブルクリックは動作しない（Chrome仕様）

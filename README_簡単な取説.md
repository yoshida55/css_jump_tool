...
### 13. PHPジャンプ（WordPress/Local対応）【2026-04-10追加】

- **操作**: `Alt+クリック`（ブラウザ上）
- **前提**: VS Codeでテーマフォルダを開いていること・css-to-html-jumper拡張が起動していること
- **setup.bat不要**（VS Code拡張のHTTPサーバー経由で動作）

#### 2つのモード

| モード | 方法 | 精度 |
|---|---|---|
| **自動検索**（推奨） | そのまま Alt+クリック | タグ名＋クラスで検索・URLで優先ファイルを自動判定 |
| **正確モード** | タグに `<?= php_src() ?>` を追加 | 行番号まで正確 |

#### 自動検索の使い方

```
① VS Codeでテーマフォルダを開く
② ブラウザ（yoursite.local 等）で要素を Alt+クリック
③ VS CodeでPHPファイルが開いてその行が黄色くハイライトされる
```

#### 正確モードの使い方（行番号まで正確に飛びたい場合）
...
`.local` ドメイン（Local by Flywheel）対応済み。

---

### 14. HTMLハイライト（ブラウザ → VS Code）【2026-02-12追加】

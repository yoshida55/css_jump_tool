...
| 機能 | 操作 | 説明 |
|------|------|------|
| CSSジャンプ | Alt+クリック / ダブルクリック | 要素のCSS定義をVS Codeで開く |
| **モバイルCSS検索** | **Ctrl+ダブルクリック / 767px以下** | **@media内のCSSを優先検索** |
| **HTMLハイライト** | **上記ジャンプ時** | **HTMLファイルの該当行を黄色表示（3秒）** |
| ブラウザハイライト | VS Codeでセレクタ選択 | VS Codeでセレクタ行にカーソル→ブラウザで該当要素をハイライト |
| Flex情報表示 | 自動 / 右クリックメニュー / **Alt+F** | Flexbox階層をラベルで可視化 |
...
|------|------|
| **Alt+クリック** | 通常のCSSジャンプ |
| **ダブルクリック** | CSS + HTML🟡 + ブラウザ🔴（3点連携） |
| **Ctrl+ダブルクリック** | 📱強制モバイルCSS検索 |
| **Flexラベルクリック** | モバイル自動検知対応 |

### 3点連携（ダブルクリック時）
1. **CSSファイル** → VS Codeで開く（フォーカス）
2. **HTMLファイル** → VS Codeで開く（🟡該当行を黄色ハイライト3秒）
3. **ブラウザ** → 該当要素を🔴赤枠ハイライト（3秒）

### モバイルCSS自動検知（2026-02-12追加）
- **ビューポート幅 767px以下** → 自動的に `@media (max-width: 767px)` 内を優先検索
- DevToolsのレスポンシブモード対応
- **Ctrl+ダブルクリック** → PC表示時も強制的にモバイルCSS検索

⚠ **DevToolsレスポンシブモード中のダブルクリックは動作しない**（Chrome仕様）
→ レスポンシブモード中は **Flexラベルクリック** または **Alt+クリック** を使用

### 検索優先度
1. ID（`#header`）→ 最優先
2. 最初のクラス名（`.nav_bar`）
3. その他クラスで再検索（フォールバック）
4. **モバイル時（767px以下）** → `@media (max-width: 767px)` 内を優先

### 仕組み
```
Alt+クリック/ダブルクリック → content.js → background.js（CSS検索）
→ Native Messaging → open_vscode.exe → vscode://file/path:行番号
→ VS Code拡張 → HTMLハイライト（/highlight-line エンドポイント経由）
```

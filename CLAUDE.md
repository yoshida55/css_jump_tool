...
| **PHPからCSSジャンプ** | **Ctrl+Click（PHPファイル）**               | **`class="..."` 内のクラス名からCSS定義行へジャンプ。複数候補はネイティブPickerで選択**            |
| **CSS未定義クラス波線** | **PHPファイル保存時（Ctrl+S）**            | **CSSに存在しないクラス名に黄色い波線（右側マーカー・Problemsパネルなし）**                        |
| **PHP括弧チェック** | **PHPファイル保存時（Ctrl+S）**                  | **対応する括弧がない箇所を行末に表示（文字列・コメント内は無視）**                                  |
| **PHP関数名ヒント** | **PHPファイル保存時（Ctrl+S）**                  | **`est_html` → `🔍 もしかして: esc_html ?` を行末に表示（150個のWP/PHP関数と照合）**             |
| **🤖 AIコードチェック** | **選択あり + 手動Ctrl+S（PHPファイル）**    | **選択行をGemini Flash LiteでWordPress/PHPレビュー。1件は行上に、複数件は行末に横並び表示**        |

### PHP開発支援機能（2026-04-11追加）

#### PHPからCSSジャンプ（Ctrl+Click）

- PHPテンプレートの `class="foo bar"` にカーソルを置いてCtrl+Click → CSSの定義行へジャンプ
- 複数候補はVS Codeネイティブのピークビュー（インライン候補一覧）で表示
- 動的クラス（`<?= $var ?>`）は無視してスキップ
- `provideDefinition` で実装（HTMLと同一の仕組み）

#### CSS未定義クラス波線

- PHPまたはHTMLファイルの保存時（手動・自動どちらも）に実行
- `class="..."` 内のクラス名をすべてのCSSファイルと照合
- 存在しないクラス名に **黄色い波線**（`textDecoration: underline wavy`）
- 右側のスクロールバーマーカー・Problemsパネルには出ない（邪魔にならない設定）

#### PHP括弧チェック

- PHPファイルの保存時に `(`/`)`/`{`/`}`/`[`/`]` の対応をスタックで検証
- 文字列（`'...'` `"..."`）とコメント（`//` `#` `/* */`）の内部はスキップ
- 対応しない括弧がある行末に `⚠ この ( に対応する ) がありません` と表示

#### PHP関数名ヒント（もしかして）

- 保存時にPHPの関数名をWordPress/PHP関数150個リストと照合
- Levenshtein距離1〜2の近い関数名が見つかれば行末に `🔍 もしかして: esc_html ?` を表示
- 対象カテゴリ: エスケープ / テンプレートタグ / 条件分岐 / エンキュー / フック / 翻訳 / オプション / ACF / PHP標準など

#### AIコードチェック（Gemini Flash Lite）

- **手動Ctrl+S のみ**実行（自動保存はスキップ）
- **選択範囲なし** → AIは動かない
- **選択範囲あり** → 選択行またはその周辺の関数ブロック全体をGemini Flash LiteでWordPress/PHPレビュー
- 1件の指摘 → `before` 装飾で行の上（コード左側）に表示
- 複数件が同じ行 → `after` 装飾で行末に横並び（` ｜ ` 区切り）
- ステータスバーに結果件数を表示（次の手動保存までクリアしない）
- プロンプト知識: 直接出力関数（the_title等）/ the_content()の安全性 / 確実なバグ・セキュリティ問題のみ指摘（推奨レベルは除外）
...
## Alt+X（XDデザインカンプ取得）（2026-03-25 完成）

### 機能概要
Adobe XD共有ページ（xd.adobe.com）でテキスト要素をクリックするたびに自動でフォント情報を蓄積し、Alt+X でパネル表示 → JSONコピー → Claude Codeへ投げてCSS一括更新できる機能。

### 使い方
1. XDの共有ページを開く
2. デザインカンプ上の要素を**クリックしていく**（何もしなくていい、自動でキャプチャ）
3. 左上に「📸 N件キャプチャ済」バッジが出る
4. 全部クリックし終わったら **Alt+X** でパネルを開く
5. **「📋 JSONをコピー（Claude Code用）」**ボタンを押す
6. Claude Codeに貼り付けて「このJSONでCSSのフォントサイズを更新して」と伝える

### パネルのボタン
| ボタン | 動作 |
|--------|------|
| 📋 JSONをコピー（Claude Code用） | 蓄積データをJSON形式でクリップボードにコピー |
| 🗑 クリア | 蓄積データをリセット |
...
### 実装の仕組み（content.js）
- `MutationObserver` でXDの右パネル（スタイル・コンテンツセクション）の変化を監視
- 要素クリック → 右パネルが更新 → `extractFromStyleSection()` + `extractContentText()` で正確な値を取得
- `xdCaptured[]` 配列に蓄積（同じテキストは上書き）
- ネットワーク解析ではなくDOMから読むため値が正確

### ⚠ 注意
- ページをリロードするとキャプチャデータはリセットされる
- XD共有ページ（xd.adobe.com）専用。他のサイトでは動作しない
- MutationObserverのデバウンスは400ms（連打しても問題なし）

### 過去に試してだめだったこと（参考）
- ネットワーク傍受（webRequest / fetch横取り）：XDのデータがRIFFバイナリ形式で来るため正確に取れなかった
- JSON.parseでのツリー走査：rawTextとfontSizeが別ノードにあり紐付けできなかった

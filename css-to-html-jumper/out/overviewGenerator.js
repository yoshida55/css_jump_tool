...
exports.registerOverviewGenerator = registerOverviewGenerator;
...
let overviewStatusBar = null;
// 現在監視中のソースファイルパス
let watchedSourcePath = null;
/**
 ...
 */
function readStoredHash(htmlPath) {
    try {
 ...
 */
function updateOverviewStatusBar(sourcePath, sourceCode) {
...
function registerOverviewGenerator(context) {
    const generatorCommand = vscode.commands.registerCommand('cssToHtmlJumper.generateOverview', async (uri) => {
    ...
    const saveWatcher = vscode.workspace.onDidSaveTextDocument((doc) => {
        if (!doc.uri.fsPath.endsWith('.js') && !doc.uri.fsPath.endsWith('.ts')) {
            return;
        }
...
async function generateOverview(doc) {
...
// HTML テンプレート生成
// ========================================
function buildHtmlOverview(fileName, lineCount, data, sourceCode, sourceHash = '', generatedAt = '') {
    const functionsHtml = generateLeftPanelHtml(data);
    const fnMapJson = JSON.stringify(data.functions.map((f) => ({
        name: f.name,
        start: f.start,
        end: f.end
    })));
...
function generateLeftPanelHtml(data) {
    let html = '';
    ...
    if (data.flowGroups) {
        ...
        data.flowGroups.filter((g) => g.isPrimary).forEach((group) => {

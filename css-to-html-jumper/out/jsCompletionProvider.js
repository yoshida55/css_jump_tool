...
exports.extractJsFunctionsFromMemo = extractJsFunctionsFromMemo;
exports.getCachedJsFunctions = getCachedJsFunctions;
exports.registerJsCompletionProvider = registerJsCompletionProvider;
exports.registerJsInlineCompletionProvider = registerJsInlineCompletionProvider;
const vscode = require("vscode");
const fs = require("fs");
const jsProperties_1 = require("./jsProperties");
// メモから抽出したJS関数キャッシュ
let cachedJsFunctions = null;
/**
 * メモファイルからJS関数名を抽出する
 */
function extractJsFunctionsFromMemo(memoContent) {
    const functions = [];
    const seen = new Set();
    ...
    for (let i = 0; i < lines.length; i++) {
        ...
        while ((match = lineRegex.exec(line)) !== null) {
            ...
            if (jsProperties_1.jsMethods[name]) {
                continue;
            }
            seen.add(name);
...
function getCachedJsFunctions() {
    return cachedJsFunctions;
}
/**
 * JS/TSファイル用の途中一致補完プロバイダーを登録する
 */
function registerJsCompletionProvider(context) {
    const provider = vscode.languages.registerCompletionItemProvider(['javascript', 'typescript', 'javascriptreact', 'typescriptreact'], {
        provideCompletionItems(document, position) {
            ...
            const memoNames = new Set(memoItems.map(i => i.label));
            const dictItems = Object.entries(jsProperties_1.jsMethods)
 ...
 */
function registerJsInlineCompletionProvider(context) {
    const provider = vscode.languages.registerInlineCompletionItemProvider([
        ...
        provideInlineCompletionItems(document, position) {
            ...
            // 辞書から前方一致で候補を探す（短い順）
            const candidates = Object.entries(jsProperties_1.jsMethods)
...
//# sourceMappingURL=jsCompletionProvider.js.map
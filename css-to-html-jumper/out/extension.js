...
// Claude API 呼び出し関数
// ========================================
async function askClaudeAPI(code, question, htmlContext, isStructural) {
    const config = vscode.workspace.getConfiguration('cssToHtmlJumper');
...
3. 「# 主な変更点」としてまとめ`, showBeside: true }
    ...
    const saveListener = vscode.workspace.onDidSaveTextDocument((document) => {
        ...
        if (pendingQuizEvaluation && document.fileName.endsWith('クイズ回答.md')) {
            vscode.window.showInformationMessage('💡 クイズの評価がまだです。評価してから次の問題に進みますか？', '評価する', '後で').then(selected => {
                if (selected === '評価する') {
                    vscode.commands.executeCommand('cssToHtmlJumper.evaluateLastQuiz');
                }
            });

...
const wpFunctions_1 = require("./wpFunctions");
 ...
 */
async function expandQueryAndSelectSections(query, sections, apiKey, modelPath, lines) {
...
{"interpretations": ["解釈1", "解釈2", "解釈3"], "indices": [0, 3, 7]}`;
    ...
    try {
        ...
        const expandedQueries = Array.isArray(obj.interpretations)
            ? obj.interpretations.filter((s) => typeof s === 'string').slice(0, 3)
            : [];
        const selectedIndices = Array.isArray(obj.indices)
            ? obj.indices.filter((n) => typeof n === 'number')
            : [];
...
async function handleMemoSearch() {
    ...
    let memoInputQp;
    {
            ...
            headingItems = tmpLines
                ...
                const keyword = (asciiMatch && asciiMatch[0].trim().length > 1)
                    ...
                    : fullTitle
                        ...
                        .trim() || fullTitle;
                return { label: keyword, description: '見出し', detail: fullTitle, _lineNum: x.idx };
            });
        ...
        query = await new Promise((resolve) => {
            ...
            qp.onDidAccept(async () => {
                ...
                if (sel && (sel.description === '見出し' || sel.description === 'メモ') && sel._lineNum !== undefined) {
    ...
    try {
        ...
        const headingMatches = allMemoLines
            .map((line, idx) => ({ line, idx }))
        ...
        if (mergedResults.length < 4) {
            ...
            const supplements = allMemoLines
...
// Claude API 呼び出し関数
// ========================================
async function askClaudeAPI(code, question, htmlContext, isStructural, isHtmlGeneration, isSectionQuestion, imageData, modelOverride, useThinking) {
    const config = vscode.workspace.getConfiguration('cssToHtmlJumper');
    const apiKey = config.get('claudeApiKey', '');
    const model = modelOverride || config.get('claudeModel', 'claude-sonnet-4-5-20250929');
    if (!apiKey) {
...
async function findHtmlUsage(selectors) {
    if (selectors.length === 0)
        return '';
    const config = vscode.workspace.getConfiguration('cssToHtmlJumper');
    const targetPattern = config.get('targetFiles', '**/*.{html,php}');
                    ...
                    try {
                        ...
                        for (const phpFile of phpFiles) {
                            ...
                            for (let i = 0; i < lines.length; i++) {
                                ...
                                // クラスチェック（最初のクラス名で検索）
                                if (classes.length > 0) {
                                    const matched = classes.some(cls => line.includes('class="' + cls + '"') ||
...
`;
    ...
    const cssSelectorHoverProvider = vscode.languages.registerHoverProvider({ scheme: 'file', language: 'css' }, {
        async provideHover(document, position) {
            ...
            const visibleHtml = vscode.window.visibleTextEditors.find(e => (e.document.languageId === 'html' || e.document.languageId === 'php') &&
...
【出力】HTMLコードのみ`, showBeside: false }
    ...
    const claudeCommand = vscode.commands.registerCommand('cssToHtmlJumper.askClaude', async () => {
        ...
        // メモ見出しをサジェスト候補に追加
        let memoHeadingItems = [];
        const memoFilePathForI = vscode.workspace.getConfiguration('cssToHtmlJumper').get('memoFilePath', '');
        if (memoFilePathForI) {
            try {
        ...
        const userInput = await new Promise((resolve) => {
            ...
            qp.onDidChangeValue(value => {
                ...
                const descMatch = allSuggestItems.filter(i => i.description !== '見出し' &&
        ...
        const result = await new Promise((resolve) => {
            ...
            quickPick.onDidAccept(async () => {
                ...
                if (selected && selected.label.includes('直接質問')) {
                    ...
                    // PHPファイルは専用プロンプトに分岐
                    if (lang === 'php') {
                        const fullText = (0, wpFunctions_1.annotatePhpCode)(rawFullText); // wpFunctionsアノテーション付与
...
上記の前回の会話を踏まえて、追加の質問に日本語で答えてください。`;
        ...
        if (isCrossThink) {
            ...
            const allFiles = await vscode.workspace.findFiles('**/*.{html,css,js,ts,php,json,md}', '{**/node_modules/**,**/.git/**,**/dist/**,**/build/**,**/vendor/**,**/.cssjumper_cache/**}');
...
["ファイルパス1", "ファイルパス2", ...]`;
                        ...
                        response.on('end', () => {
                            try {
                                ...
                                if (match) {
                                    ...
                                    stage1Selected = parsed.filter((f) => fileList.includes(f));
                                }
                            }
                            catch (e) {
                                // パース失敗時は空のまま
            ...
            if (stage1Selected.length === 0) {
                ...
                stage1Selected = fileList.filter(f => f === currentRelPath ||
                    path.dirname(f) === path.dirname(currentRelPath)).slice(0, 5);
            }
...
${crossThinkQuestion}`;
        ...
        await vscode.window.withProgress({
            ...
            try {
                ...
                if (isSkeleton) {
                    ...
                    // 前回の装飾をクリア
                    for (const deco of phpReviewDecoMap.values()) {
                        deco.dispose();
                    }
                    ...
                    for (const rLine of responseLines) {
                        ...
                        const deco = vscode.window.createTextEditorDecorationType(prevLineEmpty
    ...
    // - ['class' => '...'] → 常に当プロバイダーが処理
    const phpToCssDefinitionProvider = vscode.languages.registerDefinitionProvider([{ scheme: 'file', language: 'php' }, { scheme: 'file', language: 'html' }], {
        async provideDefinition(document, position) {
            const line = document.lineAt(position.line).text;
            ...
            const cssPattern = selectorType === 'class'
                ...
                : new RegExp(`#${escapeRegex(word)}[\\s{,(\\[]`);
            // プラグイン・コアのCSSを除外して検索
            const cssUris = await vscode.workspace.findFiles('**/*.css', '**/{node_modules,wp-includes,wp-admin,plugins,bkup,backup,_bkup,_backup}/**');
            const locations = [];
            for (const cssUri of cssUris) {
                if (cssUri.fsPath.endsWith('.min.css')) {
                    continue;
                }
                try {
                    const cssDoc = await vscode.workspace.openTextDocument(cssUri);
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        ...
                        if (/@media\b/.test(line)) {
                            mediaDepth = braceDepth + (line.match(/{/g) || []).length;
                        }
                        ...
                        if (cssPattern.test(line)) {
                            ...
                            const hasProps = (() => {
                                ...
                                if (closeIdx !== -1) {
                                    return line.slice(openIdx + 1, closeIdx).includes(':');
                                }
                            ...
                            if (inMedia && !spMatch) {
                                spMatch = new vscode.Location(cssUri, new vscode.Position(i, 0));
                            }
                            if (!inMedia && !pcMatch) {
                                pcMatch = new vscode.Location(cssUri, new vscode.Position(i, 0));
                            }
                        }
                        braceDepth -= (line.match(/}/g) || []).length;
                        if (mediaDepth > 0 && braceDepth <= mediaDepth) {
                            mediaDepth = 0;
                        }
                    }
            ...
            const sortedUris = [...cssUris].sort((a, b) => {
                ...
                if (aName === phpBaseName) {
                    return -1;
                }
                if (bName === phpBaseName) {
                    return 1;
                }
                return 0;
            });
...
（あれば記載）`;
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(doc => runPhpCssCheck(doc)), vscode.workspace.onDidCloseTextDocument(doc => {
        const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
        if (editor) {
    ...
    function runPhpFunctionCheck(doc) {
        ...
        while ((m = funcCallRegex.exec(text)) !== null) {
            const funcName = m[1];
            if (PHP_KEYWORDS.has(funcName)) {
                continue;
            }
            if (WP_PHP_FUNCTIONS.includes(funcName)) {
                continue;
            }
            ...
            const diag = new vscode.Diagnostic(new vscode.Range(start, end), `もしかして: ${suggestion} ?`, vscode.DiagnosticSeverity.Hint);
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(doc => runPhpFunctionCheck(doc)), vscode.workspace.onDidCloseTextDocument(doc => {
        phpFunctionCheckDiag.delete(doc.uri);
        const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
        if (editor) {
            editor.setDecorations(phpFunctionLineDecorationType, []);
        }
    }));
    // PHP 括弧対応チェック（保存時のみ）
    ...
    function runPhpBracketCheck(doc) {
        if (doc.languageId !== 'php') {
            return;
        }
        const text = doc.getText();
        ...
        for (const item of unmatched) {
            ...
            const diag = new vscode.Diagnostic(new vscode.Range(pos, new vscode.Position(pos.line, pos.character + 1)), msg, vscode.DiagnosticSeverity.Warning);
            diag.source = 'CSS Jumper';
            diagnostics.push(diag);
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(doc => runPhpBracketCheck(doc)), vscode.workspace.onDidCloseTextDocument(doc => {
        phpBracketCheckDiag.delete(doc.uri);
        const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
        if (editor) {
            editor.setDecorations(phpBracketLineDecorationType, []);
        }
    }));
    ...
    function runPhpStructureCheck(doc) {
        if (doc.languageId !== 'php') {
            return;
        }
        ...
        while (pos < fullText.length) {
            ...
            // PHPブロック開始
            if (!inPhp && fullText.startsWith('<?php', pos)) {
                inPhp = true;
                pos += 5;
                continue;
            }
            if (!inPhp && fullText.startsWith('<?', pos)) {
                inPhp = true;
                pos += 2;
                continue;
            }
        ...
        for (let i = 0; i < lines.length; i++) {
            const line = lines[i];
            if (/while\s*\(/.test(line) && !/endwhile/.test(line)) {
                whileDepth++;
            }
            ...
            if (whileDepth > 0) {
                if (/get_footer\s*\(/.test(line)) {
                    hints.set(i, '  ⚠ get_footer() が while ループ内にあります');
                }
                if (/get_header\s*\(/.test(line)) {
                    hints.set(i, '  ⚠ get_header() が while ループ内にあります');
                }
            }
        }
        // デコレーション適用
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(doc => runPhpStructureCheck(doc)), vscode.window.onDidChangeActiveTextEditor(editor => {
        if (editor) {
            runPhpStructureCheck(editor.document);
        }
    }));
    // PHPファイルレビュー インライン表示用（行ごとに管理）
    const phpReviewDecoMap = new Map();
    let phpReviewUri;
    context.subscriptions.push(vscode.workspace.onDidChangeTextDocument(e => {
        if (!phpReviewUri || e.document.uri.toString() !== phpReviewUri) {
            return;
        }
        for (const change of e.contentChanges) {
            ...
            if (deco) {
                deco.dispose();
                phpReviewDecoMap.delete(lineNum);
            }
        }
    }));
    // ─────────────────────────────────────────────────────────────────
    ...
    context.subscriptions.push(vscode.workspace.onDidSaveTextDocument(async (doc) => {
        ...
        const annotatedText = (0, wpFunctions_1.annotatePhpCode)(rawText);
                    ...
                    res.on('end', () => {
                        try {
                            ...
                            if (text) {
                                ...
                                reject(new Error(json.error?.message || 'APIエラーが発生しました'));
                            }
                        }
                        catch {
...
function matchesSelectorInLine(line, type, name) {
    const esc = escapeRegex(name);
    if (type === 'class') {
        // ① 静的 class="... name ..."
        if (new RegExp(`class\\s*=\\s*["'][^"']*\\b${esc}\\b[^"']*["']`, 'i').test(line)) {
            return true;
        }
        ...
        // ③ PHP配列 ['class' => ['name']]
        if (new RegExp(`['"]class['"]\\s*=>\\s*\\[[^\\]]*['"]${esc}['"]`, 'i').test(line)) {
            return true;
        }
        // ④ PHPの文字列リテラル単独（echo 'name' など）
        if (new RegExp(`['"]\\s*${esc}\\s*['"]`, 'i').test(line)) {
            return true;
        }
        return false;
    }
...
function runCssDupCheck(doc) {
    ...
    for (const rule of rules) {
        ...
        if (!pseudoMap.has(base)) {
            pseudoMap.set(base, new Set());
        }
...
function expandToSurroundingBlock(doc, cursorLine) {
    const lines = doc.getText().split('\n');
    const FALLBACK = 3;

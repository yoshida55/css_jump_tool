...
import { cssProperties, analyzeValue, validCssProperties } from './cssProperties';
import { jsMethods } from './jsProperties';
import { registerAiHoverProvider } from './aiHoverProvider';
import { annotatePhpCode, WP_FUNCTION_MAP } from './wpFunctions';
...
interface MemoSection {
  ...
  description: string;  // 見出し直後の最初のテキスト行
  codePreview: string;  // コードブロックの先頭1行
  code: string;         // コードブロックの全内容
  line: number;         // memo.md 内の行番号
}
let memoSuggestIndex: MemoSection[] = [];

function buildMemoIndex(memoFilePath: string): void {
      ...
      while (j < lines.length && /^#{2,3} /.test(lines[j]) === false) {
        ...
        if (lines[j].startsWith('```') && !code) {
          ...
          while (j < lines.length && !lines[j].startsWith('```')) {
            if (lines[j].trim()) { codeLines.push(lines[j]); }
            j++;
          }
          code = codeLines.join('\n');
        }
        j++;
        if (j < lines.length && /^#{2,3} /.test(lines[j])) { break; }
      }
      if (heading) {
        sections.push({
          heading,
...
async function handleMemoSuggest() {
  ...
  await vscode.window.withProgress(
    { location: vscode.ProgressLocation.Notification, title: `💡 「${query}」の関連メモを検索中...` },
    async () => {
      ...
      const parsed = text.split('\n')
        ...
        .map((line: string) => {
          ...
          if (isNaN(idx) || idx >= memoSuggestIndex.length) { return null; }
          const parts = m[2].split('|');
          return { idx, summary: parts[0].trim(), reason: (parts[1] || '').trim() };
        })
        .filter((x: any): x is { idx: number; summary: string; reason: string } => x !== null);
  ...
  let memoInputQp: vscode.QuickPick<vscode.QuickPickItem> | undefined;
  {
    ...
    try {
      ...
      headingItems = tmpLines
        ...
        .map(x => {
          const fullTitle = x.line.replace(/^#+\s*/, '').trim();
          // ASCII始まりなら日本語が出てくる前の部分をキーワードとする
          const asciiMatch = fullTitle.match(/^[a-zA-Z0-9_\-\.:\(\)\/\s]+/);
          const keyword = (asciiMatch && asciiMatch[0].trim().length > 1)
            ? asciiMatch[0].trim()
            : fullTitle
                .replace(/とは何か.*$/, '').replace(/とは.*$/, '')
                .replace(/について.*$/, '').replace(/の使い方.*$/, '')
                .replace(/の違い.*$/, '').replace(/の仕組み.*$/, '')
                .replace(/の種類.*$/, '').replace(/の設定.*$/, '')
                .replace(/する方法.*$/, '').replace(/が[^a-zA-Z0-9].*$/, '')
                .trim() || fullTitle;
          return { label: keyword, description: '見出し', detail: fullTitle, _lineNum: x.idx } as vscode.QuickPickItem & { _lineNum: number };
        });
    ...
    query = await new Promise<string>((resolve) => {
      ...
      let lastInsertedFromHeading = '';

      qp.onDidChangeValue(value => {
        // 見出し選択でプログラム的に設定した値は1回スキップ（再候補表示を防ぐ）
        if (value === lastInsertedFromHeading) {
          lastInsertedFromHeading = '';
          return;
        }
      ...
      qp.onDidAccept(async () => {
        ...
        // 見出し・メモ行を最優先チェック（PHP関数名と一致しても補完扱い）
        if (sel && (sel.description === '見出し' || sel.description === 'メモ') && (sel as any)._lineNum !== undefined) {
          suppressMemoItems = true;
          lastInsertedFromHeading = sel.label + ' ';
          qp.value = sel.label + ' ';
          qp.items = [];
        ...
        if (sel && qp.items.length > 0) {
          ...
          } else if (sel.description === 'メモ' || sel.description === '見出し') {
            // メモ/見出し候補が表示中でもEnterは検索として確定
            accepted = true;
            resolve(qp.value.trim());
            qp.hide();
          } else {
...
async function handleBatchCategorize() {
  ...
  try {
    ...
    // 未分類の見出しを抽出
    const uncategorized: { index: number; title: string; preview: string }[] = [];
    for (let i = 0; i < lines.length; i++) {
    ...
    await vscode.window.withProgress({
      ...
      for (let i = 0; i < uncategorized.length; i += BATCH_SIZE) {
        ...
        const titleList = batch.map((h, localIdx) =>
          h.preview ? `${localIdx}: ${h.title}（内容: ${h.preview}）` : `${localIdx}: ${h.title}`
        ).join('\n');
...
async function findHtmlUsage(selectors: string[]): Promise<string> {
  ...
  const config = vscode.workspace.getConfiguration('cssToHtmlJumper');
  const targetPattern = config.get<string>('targetFiles', '**/*.{html,php}');
    ...
    try {
      ...
      for (let i = 0; i < lines.length; i++) {
        ...
        for (const selector of selectors) {
          if (matchesSelectorInLine(lines[i], 'class', selector) ||
              matchesSelectorInLine(lines[i], 'id', selector)) {
...
function extractPhpSelectors(phpText: string): string[] {
...
function findFunctionsPhp(startDir: string): string | null {
  let dir = startDir;
  for (let i = 0; i < 5; i++) {
...
async function handlePhpCssSkeleton(editor: vscode.TextEditor): Promise<void> {
  ...
  if (foundCssFolder) {
    ...
    const choice = await vscode.window.showInformationMessage(
      'css/ フォルダが見つかりません。どこに CSS を作成しますか？',
      { modal: true },
      'css/ を新規作成', 'PHPと同じフォルダ', 'キャンセル'
    );
    if (!choice || choice === 'キャンセル') { return; }
  ...
  if (cssExists) {
    const answer = await vscode.window.showInformationMessage(
      `${path.basename(cssPath)} が既に存在します。不足しているセレクタを追記しますか？`,
      { modal: true },
      'はい', 'いいえ'
    );
    if (answer !== 'はい') { return; }
  ...
  if (!functionsPhpPath) { return; }

  const functionsContent = fs.readFileSync(functionsPhpPath, 'utf8');
  const handle = phpBaseName.replace(/_/g, '-');
  if (functionsContent.includes(`'${handle}'`) || functionsContent.includes(`"${handle}"`)) { return; }
  ...
  if (funcOpenBrace >= 0) {
    ...
    while (idx < functionsContent.length && depth > 0) {
      if (functionsContent[idx] === '{') { depth++; }
      else if (functionsContent[idx] === '}') { depth--; }
      idx++;
    }
      ...
      if (req.url === '/selector') {
        ...
        req.on('end', () => {
          try {
            ...
            const type: string = data.type || 'class';
            const occurrenceIndex: number = data.occurrenceIndex || 1;
...
`;
  ...
  function startHighlightServer(port: number = 3848) {
      ...
      async provideHover(document, position) {
        ...
        for (const fileUri of htmlFiles) {
          try {
            ...
            for (let i = 0; i < lines.length; i++) {
              if (matchesSelectorInLine(lines[i], selectorType, selectorName)) {
                results.push({
                  uri: fileUri,
                  line: i,
        ...
        // 優先順位: ①画面に見えているHTML/PHP → ②CSSと同グループのHTML/PHPタブ → ③最初のマッチ
        let firstResult = results[0];

        // ①画面に見えているHTML/PHPエディタ（分割表示中）
        const visibleHtml = vscode.window.visibleTextEditors.find(
          e => (e.document.languageId === 'html' || e.document.languageId === 'php') &&
        ...
        if (visibleHtml) {
          ...
          // ②CSSエディタと同じタブグループ内のHTML/PHPタブ（アクティブなものを優先）
          ...
          if (cssEditor) {
            ...
            if (cssGroup) {
              for (const tab of cssGroup.tabs) {
                const tabFsPath = tab.input instanceof vscode.TabInputText ? tab.input.uri.fsPath.toLowerCase() : '';
                if (tabFsPath.endsWith('.html') || tabFsPath.endsWith('.php')) {
                  const match = results.find(r => r.uri.fsPath.toLowerCase() === tabFsPath);
                  if (match) {
                    firstResult = match;
                    break;
                  }
...
【出力】HTMLコードのみ`, showBeside: false }
  ...
  const claudeCommand = vscode.commands.registerCommand('cssToHtmlJumper.askClaude', async () => {
    ...
    // メモ見出しをサジェスト候補に追加
    let memoHeadingItems: vscode.QuickPickItem[] = [];
    const memoFilePathForI = vscode.workspace.getConfiguration('cssToHtmlJumper').get<string>('memoFilePath', '');
    if (memoFilePathForI) {
      try {
        const lines = fs.readFileSync(memoFilePathForI, 'utf-8').split('\n');
        memoHeadingItems = lines
          ...
          .map(line => {
            ...
            const keyword = (asciiMatch && asciiMatch[0].trim().length > 1)
              ...
              : fullTitle
                  ...
                  .replace(/する方法.*$/, '').replace(/が[^a-zA-Z0-9].*$/, '')
                  .trim() || fullTitle;
            return { label: keyword, description: '見出し', detail: fullTitle };
          })
          .filter((item, idx, arr) => arr.findIndex(i => i.label === item.label) === idx); // 重複除去
      } catch { /* メモなし */ }
    }

    const allSuggestItems = [...cssItems, ...extraCssItems, ...jsItems, ...phpFunctionItems, ...memoHeadingItems];
    ...
    const userInput = await new Promise<string | undefined>((resolve) => {
      ...
      qp.onDidChangeValue(value => {
        ...
        // 説明文一致：見出しはlabel一致のみ（detailの内容でマッチすると意図しない上書きが起きるため）
      ...
      qp.onDidAccept(() => {
        ...
        if (sel && qp.items.length > 0) {
          // 候補あり → 最後の単語を置き換えて続けて入力可能に
          const parts = qp.value.split(/[\s　]+/);
          parts[parts.length - 1] = sel.label;
          qp.value = parts.join(' ') + ' ';
          qp.items = [];
          // 古い選択をクリア（次のEnterで再置換されないよう）
          (qp as any).selectedItems = [];
        } else {
          // 候補なし → 確定して送信
          accepted = true;
...
${rawFullText}`;
          ...
          const attach = await vscode.window.showQuickPick(
            ['📂 XDデータを添付する', '⏭ スキップ'],
            { placeHolder: 'デザインカンプ（XD）のJSONファイルを添付しますか？' }
          );
            ...
            if (uris && uris.length > 0) {
              try {
                ...
                if (Array.isArray(xdData) && xdData.length > 0 && 'text' in xdData[0]) {
                  const xdSummary = xdData.map((d: any) =>
                    `・${d.text}  font-size:${d.fontSize || '—'}  font-family:${d.fontFamily || '—'}  font-weight:${d.fontWeight || '—'}  line-height:${d.lineHeight || '—'}  color:${d.color || '—'}`
                  ).join('\n');
    ...
    if (isCodingChallenge) {
      const geminiApiKey = vscode.workspace.getConfiguration('cssToHtmlJumper').get<string>('geminiApiKey', '');
      if (!geminiApiKey) {
        vscode.window.showErrorMessage('Gemini APIキーが設定されていません（cssToHtmlJumper.geminiApiKey）');
        return;
      }
      ...
      if (!memoContent) {
        ...
        for (let i = cursorLine; i >= 0; i--) {
          if (/^##?\s/.test(docLines[i])) { sectionStart = i; break; }
        }
        let sectionEnd = docLines.length;
        for (let i = sectionStart + 1; i < docLines.length; i++) {
          if (/^##?\s/.test(docLines[i])) { sectionEnd = i; break; }
        }
      ...
      await vscode.window.withProgress({ location: vscode.ProgressLocation.Notification, title: '🎮 問題を生成中...', cancellable: false }, async () => {
        try {
          ...
          const raw = await callGeminiApi(geminiApiKey, 'gemini-3.1-flash-lite-preview', postData);
          const parsed = JSON.parse(raw);
          const responseText: string = parsed?.candidates?.[0]?.content?.parts?.[0]?.text ?? '';
          // コードブロックからHTMLを抽出
...
${crossThinkQuestion}`;
  ...
  const disposable = vscode.commands.registerCommand('cssToHtmlJumper.findUsage', async () => {
    ...
    // 設定から検索対象ファイルパターンを取得（HTML + PHP対応）
    const config = vscode.workspace.getConfiguration('cssToHtmlJumper');
    const targetPattern = config.get<string>('targetFiles', '**/*.{html,php}');

    // ワークスペース内のHTML/PHPファイルを検索
    const htmlFiles = filterAutoGeneratedHtml(await vscode.workspace.findFiles(targetPattern, '**/node_modules/**'));

    if (htmlFiles.length === 0) {
      vscode.window.showWarningMessage('HTML/PHPファイルが見つかりません');
      return;
    }
    ...
    // 各HTML/PHPファイルを検索（PHP動的クラスパターンも含む）
    for (const fileUri of htmlFiles) {
      try {
        const document = await vscode.workspace.openTextDocument(fileUri);
        const text = document.getText();
    ...
    if (results.length === 0) {
      vscode.window.showInformationMessage(`「${selector}」はHTML/PHPで使用されていません`);
      return;
    }

    // 1件 → そのままジャンプ / 複数件 → QuickPickで選択
    let result = results[0];
    if (results.length > 1) {
      const items = results.map(r => ({
        label: `$(file-code) ${path.basename(r.uri.fsPath)}:${r.line + 1}`,
        description: r.text,
        result: r
      }));
      const picked = await vscode.window.showQuickPick(items, {
        placeHolder: `「${selector}」の使用箇所 (${results.length}件)`
      });
      if (!picked) { return; }
  ...
  const definitionProvider = vscode.languages.registerDefinitionProvider(
    { scheme: 'file', language: 'css' },
    {
      async provideDefinition(document, position) {
        ...
        // 全マッチを収集（PHP動的クラスパターンも含む / ファイルごとに最初の1件）
...
（あれば記載）`;
  ...
  async function runCssOrphanCheck(doc: vscode.TextDocument) {
    if (doc.languageId !== 'css') { return; }

    // HTML/PHPファイルの全クラス名を収集
    const usedClasses = new Set<string>();
    const htmlPhpFiles = await vscode.workspace.findFiles(
      '**/*.{html,php}',
      '**/{node_modules,wp-includes,wp-admin,plugins,vendor}/**',
      60
    );
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      while ((cm = classRegex.exec(line)) !== null) {
        ...
        if (WP_AUTO_CLASSES.has(className)) { continue; }
        if (WP_AUTO_PREFIXES.some(p => className.startsWith(p))) { continue; }
    ...
    const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
    if (editor) { editor.setDecorations(cssOrphanDecorationType, decorations); }
  }

  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(doc => runCssOrphanCheck(doc)),
    vscode.window.onDidChangeActiveTextEditor(editor => {
      if (editor) { setTimeout(() => runCssOrphanCheck(editor.document), 1000); }
    }),
    vscode.workspace.onDidCloseTextDocument(doc => {
      const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
      if (editor) { editor.setDecorations(cssOrphanDecorationType, []); }
    })
  );
  ...
  function runCssPropertyCheck(doc: vscode.TextDocument) {
    if (doc.languageId !== 'css') { return; }
    ...
    for (let i = 0; i < lines.length; i++) {
      ...
      if (!trimmed || trimmed.startsWith('@') || trimmed.includes('{') || trimmed === '}' || trimmed.startsWith('*')) { continue; }
    ...
    const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
    if (editor) { editor.setDecorations(cssPropertyWavyDecorationType, decorations); }
  }

  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(doc => runCssPropertyCheck(doc)),
    vscode.window.onDidChangeActiveTextEditor(editor => {
      if (editor) { runCssPropertyCheck(editor.document); }
    }),
    vscode.workspace.onDidCloseTextDocument(doc => {
      const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
      if (editor) { editor.setDecorations(cssPropertyWavyDecorationType, []); }
    })
  );
  ...
  // ─────────────────────────────────────────────────────────────────
  const warnIconUri = vscode.Uri.parse(
    `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3E%3Cpath fill='%23ffcc00' d='M7.56 1h.88L15 13.5H1L7.56 1zm-.88 8h1.64v1.5H6.68V9zm0-4h1.64v3H6.68V5z'/%3E%3C/svg%3E`
  );
  ...
  // 固定ルール
  const phpStaticRules: { pattern: RegExp; message: string }[] = [
  ...
  const phpWpRedirectRules: { pattern: RegExp; message: string }[] = [];
  for (const [directFunc, meta] of Object.entries(WP_FUNCTION_MAP)) {
  ...
  function runPhpRuleLintCheck(doc: vscode.TextDocument): void {
    if (doc.languageId !== 'php') { return; }
    const editor = vscode.window.visibleTextEditors.find(e => e.document === doc);
    if (!editor) { return; }
  ...
  context.subscriptions.push(
    vscode.workspace.onDidSaveTextDocument(doc => runPhpRuleLintCheck(doc)),
    vscode.window.onDidChangeActiveTextEditor(editor => {
      if (editor) { runPhpRuleLintCheck(editor.document); }
    })
  );
  if (vscode.window.activeTextEditor) {
  ...
  const aiIconUri = vscode.Uri.parse(
    `data:image/svg+xml,%3Csvg xmlns='http://www.w3.org/2000/svg' viewBox='0 0 16 16'%3E%3Ccircle cx='8' cy='8' r='7' fill='%234fc3f7'/%3E%3Crect x='7' y='7' width='2' height='5' fill='white'/%3E%3Crect x='7' y='4' width='2' height='2' fill='white'/%3E%3C/svg%3E`
  );
...
function matchesSelectorInLine(line: string, type: string, name: string): boolean {
  const esc = escapeRegex(name);
  if (type === 'class') {
    // ① 静的 class="... name ..."
    if (new RegExp(`class\\s*=\\s*["'][^"']*\\b${esc}\\b[^"']*["']`, 'i').test(line)) { return true; }
    // ② PHP配列 ['class' => 'name'] or ["class" => "name"]
    if (new RegExp(`['"]class['"]\\s*=>\\s*['"][^'"]*\\b${esc}\\b[^'"]*['"]`, 'i').test(line)) { return true; }
    // ③ PHP配列 ['class' => ['name']]
    if (new RegExp(`['"]class['"]\\s*=>\\s*\\[[^\\]]*['"]${esc}['"]`, 'i').test(line)) { return true; }
    // ④ PHPの文字列リテラル単独（echo 'name' など）
    if (new RegExp(`['"]\\s*${esc}\\s*['"]`, 'i').test(line)) { return true; }
    return false;
  } else if (type === 'id') {
    return new RegExp(`id\\s*=\\s*["']${esc}["']`, 'i').test(line);
  } else {
    // タグ名
    return new RegExp(`<${esc}[\\s>]`, 'i').test(line);
  }
}
...
function isSelectorUsed(selector: string, htmlText: string): boolean {
  ...
  for (const cls of classNames) {
    if (isClassUsed(cls, htmlText)) { continue; }
    return false;
  }
...
function isClassUsed(cls: string, htmlText: string): boolean {
  // ① 静的 class="..." 属性
  if (new RegExp(`class="[^"]*\\b${cls}\\b[^"]*"`, 'i').test(htmlText)) { return true; }
  // ② PHP配列 ['class' => 'foo bar'] or ["class" => "foo bar"]
  if (new RegExp(`['"]class['"]\\s*=>\\s*['"][^'"]*\\b${cls}\\b[^'"]*['"]`, 'i').test(htmlText)) { return true; }
  // ③ PHP add_filter / wp_nav_menu など 'class' => ['foo', 'bar'] 形式
  if (new RegExp(`['"]class['"]\\s*=>\\s*\\[[^\\]]*['"]${cls}['"]`, 'i').test(htmlText)) { return true; }
  // ④ PHPの文字列リテラル内に単独でクラス名が現れる（echo 'single_thumbnail' 等）
  if (new RegExp(`['"]\\s*${cls}\\s*['"]`, 'i').test(htmlText)) { return true; }
  return false;
}
...
function runCssDupCheck(doc: vscode.TextDocument): vscode.DecorationOptions[] {
  ...
  for (const rule of rules) {
    ...
    if (!sel.includes('::before') && !sel.includes('::after') &&
        !sel.includes(':before')  && !sel.includes(':after')) { continue; }
  ...
  let kd: RegExpExecArray | null;
  while ((kd = keyframeDefRegex.exec(text)) !== null) { definedKeyframes.add(kd[1]); }
  ...
  for (const rule of rules) {
    ...
    for (const name of nameParts) {
      ...
      if (hasIgnoreComment(doc, animInfo.offset)) { continue; }

...
interface WpFunctionMeta {
  category: 'direct_output' | 'returns_value' | 'boolean' | 'action';
  safe: boolean;          // そのまま出力して安全か
  escape?: string;        // 推奨エスケープ関数
  returnVersion?: string; // 直接出力版に対応するreturn版
  note: string;
}

export const WP_FUNCTION_MAP: Record<string, WpFunctionMeta> = {
 ...
 */
export function annotatePhpCode(code: string): string {
  const lines = code.split('\n');
  const annotated = lines.map(line => {
    const matches: string[] = [];

    for (const [funcName, meta] of Object.entries(WP_FUNCTION_MAP)) {
